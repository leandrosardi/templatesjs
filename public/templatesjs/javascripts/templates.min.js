...
var templatesJs = {
	...
	// use this global variable to remember the values written in template, even if the user go to another tamplate and then get back to this one 
	persistance: {},
	 ...
	 * update the border color of the template.
	 * 
	 */
	...
	/*
	 * Draws the template element inside a parent element
	 * 
	 * example:
	...
	draw: function(parent, h) {
		...
		// convert template-spintax to template
		...
		"	<div class='title'> ";
			...
			if ( h['versions'].length > 1) {
				...
				if (h['hidden']) {
					...
					html +=
					"	<select class='versions' style='visibility:hidden;'> ";
				}
		...
		"	</div> " +
		"	<div class='raw'> " +
		"		<textarea class='raw-code'> </textarea> " +
		"	</div> " +
		"</div> ";
	...
	/*
	 * Returns true if a template has been compalted by filling up all its blank spaces.
	 * 
	 */
	...
	setFocus: function(parent) {
		if ( templatesJs.isCodeOn(parent) === true ) {
			$(parent).find('div.raw textarea').select();
		} else {
